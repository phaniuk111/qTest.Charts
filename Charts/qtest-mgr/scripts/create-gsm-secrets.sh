...
# Jira OAuth2 — docs say qTest won't start if these are empty,
# even if you're not connecting to Jira OAuth2.
create_secret "qtest-jira-oauth2-password" "$(head -c 32 /dev/urandom | base64)"
create_secret "qtest-jira-oauth2-salt" "$(head -c 16 /dev/urandom | xxd -p)"
...
echo "  Database:    qtest-db-password, qtest-db-readonly-password"
echo "  OAuth:       9 x qtest-oauth-*-secret"
echo "  Session:     qtest-sessions-secret-key, qtest-csrf-key, qtest-csrf-secret-key"
echo "  Pulse:       qtest-pulse-executor-api-key"
echo "  Encryption:  qtest-aes-secret-keys"
echo "  Scenario:    qtest-scenario-refresh-token-secret, qtest-jira-cypher-key"
echo "  Jira OAuth2: qtest-jira-oauth2-password, qtest-jira-oauth2-salt"
echo "  Optional:    qtest-mail-password, qtest-launchdarkly-sdk-key (empty)"
echo ""
echo "NOTE: The qtest-i-etl-secret ExternalSecret reuses qtest-db-password"
echo "      for all 4 keys by default. Override in values-gke-custom.yaml"
echo "      if your i-ETL service uses separate DB credentials."
echo ""
echo "Next steps:"
